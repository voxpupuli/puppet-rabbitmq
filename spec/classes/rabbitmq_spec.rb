...
describe 'rabbitmq' do
  ...
  context 'on Debian' do
    ...
    describe 'apt::source default values' do
      it 'adds a repo with defaults values' do
  ...
  context 'on Debian' do
    ...
    describe 'apt::source default values' do
      it 'adds a repo with defaults values' do
        is_expected.to contain_apt__source('rabbitmq').with(ensure: 'absent')
      end
    end
  end
  ...
  context 'on Redhat' do
    ...
    it 'includes rabbitmq::repo::rhel' do
      ...
      is_expected.to contain_exec('rpm --import https://www.rabbitmq.com/rabbitmq-release-signing-key.asc')
    end
  ...
  context 'on Redhat' do
    ...
    it 'does not import repo public key when repos_ensure is false' do
      is_expected.to contain_class('rabbitmq::repo::rhel')
      is_expected.not_to contain_exec('rpm --import https://www.rabbitmq.com/rabbitmq-release-signing-key.asc')
    end
  end
  ...
  %w[Debian RedHat SUSE Archlinux].each do |distro|
    ...
    context "on #{distro}" do
      ...
      context 'with admin_enable set to true' do
        ...
        context 'with service_manage set to true' do
          it 'we enable the admin interface by default' do
            ...
            is_expected.to contain_staging__file('rabbitmqadmin').with_source('http://guest:guest@1.1.1.1:15672/cli/rabbitmqadmin')
          end
        end
        context 'with default $node_ip_address="UNSET" and service_manage set to true' do
          ...
          it 'we enable the admin interface by default' do
            ...
            is_expected.to contain_staging__file('rabbitmqadmin').with_source('http://guest:guest@127.0.0.1:15672/cli/rabbitmqadmin')
          end
        end
      ...
      describe 'manages configuration file correctly' do
        it { is_expected.to contain_file('rabbitmq.config') }
      end
      ...
      context 'configures config_cluster' do
        ...
        describe 'with defaults' do
          it 'fails' do
            expect { catalogue }.to raise_error(Puppet::Error, %r{You must set the \$erlang_cookie value})
          end
        end
      ...
      describe 'non-bool tcp_keepalive parameter' do
        ...
        it 'raises an error' do
          expect do
            is_expected.to contain_file('rabbitmq.config')
          end.to raise_error(Puppet::Error, %r{is not a boolean})
        end
      end
      ...
      describe 'non-integer rabbitmq-heartbeat options' do
        ...
        it 'raises a validation error' do
          expect do
            is_expected.to contain_file('rabbitmq.config')
          end.to raise_error(Puppet::Error, %r{Expected first argument to be an Integer})
        end
      end
      ...
      describe 'service with ensure neither running neither stopped' do
        ...
        it 'raises an error' do
          ...
          end.to raise_error(Puppet::Error, %r{validate_re\(\): "foo" does not match "\^\(running\|stopped\)\$"})
        end
      end
  ...
  %w[RedHat SuSE].each do |distro|
    ...
    describe "repo management on #{distro}" do
      describe 'imports the key' do
        ...
        it {
          is_expected.to contain_exec("rpm --import #{params[:package_gpg_key]}").with(
