...
describe 'Puppet::Type.type(:rabbitmq_user_permissions).provider(:rabbitmqctl)' do
  ...
  it 'does not match an empty list' do
    expect(provider.class).to receive(:rabbitmqctl_list).with('user_permissions', 'foo').and_return('')
  ...
  it 'creates default permissions' do
    ...
    expect(provider).to receive(:rabbitmqctl).with('set_permissions', '-p', 'bar', 'foo', "''", "''", "''")
    provider.create
  end
  ...
  it 'destroys permissions' do
    provider.instance_variable_set(:@should_vhost, 'bar')
    provider.instance_variable_set(:@should_user, 'foo')
    expect(provider).to receive(:rabbitmqctl).with('clear_permissions', '-p', 'bar', 'foo')
    provider.destroy
  end
  ...
  it 'onlies call set_permissions once' do
    ...
    expect(provider).to receive(:rabbitmqctl).with('set_permissions', '-p', 'bar', 'foo', 'foo', '2', 'foo').once
