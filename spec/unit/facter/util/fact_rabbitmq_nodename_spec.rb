  ...
  describe 'rabbitmq_nodename' do
    context 'with value' do
      it do
        ...
        allow(Facter::Core::Execution).to receive(:execute).with('rabbitmqctl status 2>&1').and_return('Status of node monty@rabbit1 ...')
    ...
    context 'with dashes in hostname' do
      it do
        allow(Facter::Util::Resolution).to receive(:which).with('rabbitmqctl').and_return(true)
        allow(Facter::Core::Execution).to receive(:execute).with('rabbitmqctl status 2>&1').and_return('Status of node monty@rabbit-1 ...')
    ...
    context 'with dashes in nodename/hostname' do
      it do
        allow(Facter::Util::Resolution).to receive(:which).with('rabbitmqctl').and_return(true)
        allow(Facter::Core::Execution).to receive(:execute).with('rabbitmqctl status 2>&1').and_return('Status of node monty-python@rabbit-1 ...')
    ...
    context 'rabbitmq is not running' do
      it do
        ...
        EOS
        allow(Facter::Util::Resolution).to receive(:which).with('rabbitmqctl').and_return(true)
        allow(Facter::Core::Execution).to receive(:execute).with('rabbitmqctl status 2>&1').and_return(error_string)
        expect(Facter.fact(:rabbitmq_nodename).value).to eq('monty@rabbit-1')
      end
    end
