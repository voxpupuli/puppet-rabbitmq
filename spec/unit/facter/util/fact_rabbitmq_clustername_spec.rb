  ...
  describe 'rabbitmq_clustername' do
    context 'with value' do
      it do
        ...
        Facter::Core::Execution.expects(:execute).with('rabbitmqctl cluster_status 2>&1').returns(' {cluster_name,<<"monty">>},')
    ...
    context 'with dashes in hostname' do
      it do
        Facter::Util::Resolution.expects(:which).with('rabbitmqctl').returns(true)
        Facter::Core::Execution.expects(:execute).with('rabbitmqctl cluster_status 2>&1').returns(' {cluster_name,<<"monty@rabbit-1">>},')
    ...
    context 'with dashes in clustername/hostname' do
      it do
        Facter::Util::Resolution.expects(:which).with('rabbitmqctl').returns(true)
        Facter::Core::Execution.expects(:execute).with('rabbitmqctl cluster_status 2>&1').returns(' {cluster_name,<<"monty-python@rabbit-1">>},')
...
- cookie hash: 6WdP0nl6d3HYqA5vTKMkIg==
        ...
        EOS
        Facter::Util::Resolution.expects(:which).with('rabbitmqctl').returns(true)
        Facter::Core::Execution.expects(:execute).with('rabbitmqctl cluster_status 2>&1').returns(error_string)
